			...
			<v-expansion-panel v-for="(product, index) in products" :key="index">
				<v-expansion-panel-header class="pa-3" ripple @click.stop>
					<div class="d-flex align-center justify-space-between">
						...
						<div :img="product.photo_url" class="d-flex justify-content-center align-center">
							...
							<v-tooltip bottom>
								<template v-slot:activator="{ on }">
									<v-btn v-if="editable" small icon @click.stop="removeItem(product)" color="red" v-on="on">
				...
				<v-expansion-panel-content class="pa-3">
					...
					<v-row no-gutters>
						<v-col cols="12">
							<cartDiscount
								:product_index="index"
								:product_price="product.final_price * product.qty"
								:editable="editable"
							></cartDiscount>
						</v-col>
					</v-row>
