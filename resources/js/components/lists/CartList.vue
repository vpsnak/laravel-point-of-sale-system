<template>
  <v-card>
    <v-card-text>
      ...
      <v-card>
        <v-card-title class="blue-grey text-center justify-center">
          <v-icon>shopping_cart</v-icon>
          <div class="pl-2">Shopping cart</div>
        </v-card-title>
        <v-card-text>
          <!-- <v-subheader>Select a Customer</v-subheader> -->
          <v-autocomplete
            v-model="customerSearch"
            :hint="!isEditing ? 'Click the icon to edit' : 'Click the icon to save'"
            :items="customerList"
            :readonly="!isEditing"
            :label="`Customer — ${isEditing ? 'Editable' : 'Readonly'}`"
    ...
    <v-btn @click="submitCart">Submit</v-btn>
  </v-card>
</template>
