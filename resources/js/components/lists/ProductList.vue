	...
	<v-card>
		<v-container>
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn @click="addDummyProductDialog()" v-on="on" class="ma-2">
							<v-icon left>mdi-flower</v-icon>Custom
							<br />Product
						</v-btn>
					</template>
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn @click="giftcardDialog()" v-on="on" class="ma-2">
							<v-icon left>mdi-credit-card-plus</v-icon>Gift
							<br />Card
						</v-btn>
					</template>
			...
			<v-row v-if="products.length">
				<v-container style="max-height:61vh;" class="overflow-y-auto fill-height">
					<v-col v-for="product in products" :key="product.id" :cols="12" :md="6" :lg="4">
						<v-card dark :img="product.photo_url" @click="addProduct(product)" height="170px">
							<v-card-title class="secondary pa-0" @click.stop>
								...
								<v-menu bottom left>
									...
									<v-list>
										...
										<v-list-item :disabled="!product.plantcare_pdf" @click="mailPlantCareDialog(product)">
			...
			<v-row v-else align="center" justify="center" style="height: 58vh; overflow-y: auto;">
				<v-progress-circular v-if="loader" indeterminate color="secondary"></v-progress-circular>
