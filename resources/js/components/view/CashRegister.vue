  ...
  <v-container>
    <v-row v-if="cashRegisterData">
        ...
        <v-card>
          ...
          <v-card-text>
            <div class="subtitle-1">
              Created at: {{ cashRegisterData.created_at }}
            </div>
            <div class="subtitle-1">
              Updated at: {{ cashRegisterData.updated_at }}
            </div>
          </v-card-text>
        </v-card>
      </v-col>
          ...
          <v-card-text>
            <v-simple-table dense>
              <template v-slot:default>
                ...
                <tbody>
                  <tr
                    v-for="payment in cashRegisterData.payments"
                    :key="payment.id"
                  >
      ...
      <v-col cols="12" v-else>
        <v-card-title>Payments</v-card-title>
        <v-card-text>
          There are no paymens that have been made by this cash register
        </v-card-text>
      </v-col>
    </v-row>
    <v-row v-else>
...
export default {
  ...
  mounted() {
    if (this.$props.model)
      this.request({
        method: "get",
        url: `cash-registers/get/${this.$props.model.id}`
      }).then(result => {
